...
```{r data setup, message=FALSE, warning=FALSE, out.width = '100%', results='hide', fig.keep='all'}
...
ggplot() +
  ...
  scale_fill_gradient(low = "#E91C1C", high = "lightgray", name = "Distance (Feet)") +
...
for(i in block_dat.nns){
  mapList[[i]] <-
    ...
    scale_fill_gradient(low = "#E91C1C", high = "lightgray") +
    labs(title = i) +
  mapThememin()
}
...
cost_benefit_table <-
   SVM_testProbs %>%
    ...
    bind_cols(data.frame(Description = c(
              "Predicted correctly rat detection",
              "Predicted correctly lack of rat detection",
              "Predicted rat detection, but incorrect",
              "Predicted lack of rat detection, but incorrect")))
...
- True Positive (top left): The model correctly predicts rat detection
- True Negative (bottom right): The model correctly predicts lack of rat detection
- False Positive (bottom left): The model predicts rat detection, but rats are not detected
- False Negative (top right): The model predicts lack of rat detection, but rats are detected
...
```{r cv, message=FALSE, warning=FALSE, out.width = '100%', results='hide', fig.keep='all'}
