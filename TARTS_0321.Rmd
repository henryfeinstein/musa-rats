...
ggplot() +
  geom_sf(data = block_311_monthly %>%
            filter(TYPE == "Trash_issues"), aes(fill = count), color = "transparent") +
  scale_fill_gradient(low = greyPalette5[1], high = bluePalette5[5]) +
  facet_wrap(~month) +
...
ggplot() +
  geom_sf(data = block_311_monthly %>%
            filter(TYPE == "Infrastructure_issues"), aes(fill = count), color = "transparent") +
  scale_fill_gradient(low = greyPalette5[1], high = bluePalette5[5]) +
  facet_wrap(~month) +
...
# stepwise_fit <- step(glm(activity ~ ., data = as.data.frame(training_data) %>% 
#             dplyr::select(-serviceday, -block_id, -SERVICEORDERDATE, -INSPECTIONDATE, -WARD, -year, -week, -month, -calls, -inspection_count, -rats_found_yn, -rats_found_count, -found)), direction = "both")
# 
# # Fit a zero-inflated model using the selected variables
# selected_vars <- names(stepwise_fit$coefficients)[-1]
# 
# zinf_training_data <- training_data

# zinf_fit <- zeroinfl(activity ~ selected_vars, data = zinf_training_data)
...
Precision <-
  cf_mtrix$table[2, 2] / (cf_mtrix$table[2, 2] + cf_mtrix$table[1, 2])
  
Recall <-
  cf_mtrix$table[2, 2] / (cf_mtrix$table[2, 2] + cf_mtrix$table[2, 1])
...
actual <- as.numeric(as.integer(testProbs$Outcome))
predicted <- as.numeric(as.character(testProbs$predOutcome))
...
Logit_f1 <- F1_Score(predict(reg_ALL, testing_data, type= "response"), testing_data$activity)
