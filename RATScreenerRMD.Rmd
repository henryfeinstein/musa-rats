...
ggplot() + geom_sf(data = block_dat, aes(fill = storm_drain), color = "transparent") +
  scale_fill_gradient(low = "lightgray", high = "#E91C1C", name = "Count") +
...
ggplot() + 
  geom_sf(data = block_dat, aes(fill = zoning), color = "transparent") +
  scale_fill_manual(values = c("#FFFFFF", "#E0E0E0", "#A9A9A9", "#808080","#d6baba", "#E57373", "#F44336", "#D32F2F", "#B71C1C"), name = "Zoning") +
  labs(title = "Zoning in Washington, DC") +
  mapThememin2()
  

ggplot(data=block_dat, aes(x = zoning, y = rats_found_count)) +
  geom_bar(stat="identity", width=0.5) +
  labs(title="Rat Detection Count by Zoning Type", x="Zoning", y="Count of Rat Detection Instances") +
  coord_flip() +
  plotTheme
...
ggplot() +
  ...
  scale_fill_gradient(low = "lightgray", high = "#E91C1C", name = "Count") +
  labs(title = "Count of Rat Detection Instances by Fishnet",
       subtitle = "",
       caption = "") +
  mapThememin2()
...
Below is a map of the predictions for test data blocks, as well as map with predictions aggregated to the ward level.
...
#block_dat_probs$ward <- as.factor(block_dat_probs$ward)

block_dat_probs$ward <- st_within(block_dat_probs,wards)

block_dat_probs_ward <- block_dat_probs %>%
  group_by(ward) %>%
  dplyr::summarize(mean_Probs = mean(Probs, na.rm=FALSE))

block_dat_probs_ward$predOutcome <- ifelse(block_dat_probs_ward$Probs >= 0.25, 1, 0)
block_dat_probs_ward$Outcome <- ifelse(block_dat_probs_ward$found == 'found', 1,0)
block_dat_probs_ward$Outcome <- as.character(block_dat_probs_ward$Outcome)

block_dat_probs_ward$correct <- ifelse(block_dat_probs_ward$Outcome == 1 & block_dat_probs_ward$predOutcome == 1, "True Positive", 
                                ifelse(block_dat_probs_ward$Outcome == 0 & block_dat_probs_ward$predOutcome == 0, "True Negative",
                                       ifelse(block_dat_probs_ward$Outcome == 1 & block_dat_probs_ward$predOutcome == 0, "False Negative", "False Positive")))
block_dat_probs_ward$True_Pos <- ifelse(block_dat_probs_ward$correct == "True Positive", 1, 0)
block_dat_probs_ward$True_Neg <- ifelse(block_dat_probs_ward$correct == "True Negative", 1, 0)
block_dat_probs_ward$False_Pos <- ifelse(block_dat_probs_ward$correct == "False Positive", 1, 0)
block_dat_probs_ward$False_Neg <- ifelse(block_dat_probs_ward$correct == "False Negative", 1, 0)

block_dat_probs_ward %>%
  group_by(ward) %>%
  dplyr::summarize(True_Pos = sum(True_Pos),
                   True_Neg = sum(True_Neg),
                   False_Pos = sum(False_Pos),
                   False_Neg = sum(False_Neg),
                   Sum= sum(True_Pos)+sum(True_Neg)+sum(False_Pos) +sum(False_Neg),
                   Accuracy = (sum(True_Pos) + sum(True_Neg))/Sum)

block_preds_map <- block_dat_probs %>%
  dplyr::select(block_id, Probs, geometry) %>%
  dplyr::mutate(Probs = (Probs)*100)

block_preds <- ggplot() +
  geom_sf(data=blocks, color = "transparent") +
  geom_sf(data = block_preds_map, aes(fill=Probs), color = "transparent") +
  scale_fill_gradient(low = "lightgray", high = "#E91C1C", name = "Probability (%)")+
  labs(title = "Block Predictions - Rat Detection Probability",
       caption = "") +
  mapThememin2()
  
ward_preds <- ggplot() +
  geom_sf(data = wards, color = "transparent") +
  geom_sf(data = wards, aes(fill=mean_probs), color = "white") +
  scale_fill_gradient(low = "#e3b1b1", high = "#E91C1C", name = "Probability (%)")+
  labs(title = "Average Prediction by Ward - Rat Detection Probability",
       caption = "") +
  mapThememin2()

grid.arrange(block_preds, ward_preds, ncol = 2)
```
...
whichThreshold %>%
  ...
  geom_point() +
  scale_colour_manual(values = "darkgray","#E99191", "#E91C1C") +    
...
ggplot(whichThreshold_cb, aes(Threshold, Weighted_Value)) +
  ...
  geom_vline(xintercept =  optimal_th, colour = "#E99191", linetype = 4, size = 1) +
...
![RATScreener Web App](.data/images/rats_app.png)
